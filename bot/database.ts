...
export type ApplicationType = "create" | "rank" | "delete";
...
export interface GrantPointsResult {
  ...
  issuerDailyTotal: number;
  issuerDailyRemaining: number;
}
...
export class BotDatabase {
  ...
  private migrate(): void {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS applications (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        guild_id TEXT NOT NULL,
        type TEXT NOT NULL CHECK (type IN ('create', 'rank', 'delete')),
      ...
      CREATE TABLE IF NOT EXISTS curator_points (
        guild_id TEXT NOT NULL,
        user_id TEXT NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_point_grants_issuer_day
        ON point_grants (guild_id, issuer_id, created_at);
      CREATE INDEX IF NOT EXISTS idx_point_grants_history
        ON point_grants (guild_id, created_at DESC);
    `);
    this.migrateApplicationsType();
    this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_applications_status
        ON applications (guild_id, status);
    `);
    ...
    const migrate = this.db.transaction(() => {
      this.db.exec(`
        ...
        INSERT INTO applications (
          id, guild_id, type, user_id, target_user_id, nickname, family_name,
          color_hex, rank, evidence_urls, family_id, reason, status,
          temp_channel_id, review_message_id, created_at, reviewed_by, reviewed_at
        )
        SELECT
          id, guild_id, type, user_id, target_user_id, nickname, family_name,
          color_hex, rank, evidence_urls, family_id, reason, status,
  ...
  public createApplication(input: {
    ...
    const result = this.db
      ...
      .run({
        ...
        status: input.type === "create" || input.type === "rank" ? "collecting_evidence" : "pending",
      });
  ...
  public getPointBalance(guildId: string, userId: string): PointBalance {
    const row = this.db
      .prepare("SELECT points FROM curator_points WHERE guild_id = ? AND user_id = ?")
      .get(guildId, userId) as { points?: number } | undefined;
    return { guildId, userId, points: Number(row?.points ?? 0) };
  }

  public getPointLeaderboard(guildId: string, limit = 10): PointBalance[] {
    ...
    const rows = this.db
      ...
      .all(guildId, safeLimit) as Array<{ guild_id: string; user_id: string; points: number }>;
    return rows.map((row) => ({
  ...
  public getPointHistory(guildId: string, limit = 10, recipientId?: string): PointGrant[] {
    const safeLimit = Math.max(1, Math.min(50, Math.trunc(limit)));
    const rows = recipientId
      ? this.db
          .prepare(
            `SELECT id, guild_id, issuer_id, recipient_id, amount, created_at
             FROM point_grants
             WHERE guild_id = ? AND recipient_id = ?
             ORDER BY id DESC
             LIMIT ?`,
          )
          .all(guildId, recipientId, safeLimit)
      : this.db
          .prepare(
            `SELECT id, guild_id, issuer_id, recipient_id, amount, created_at
             FROM point_grants
  ...
  public grantPoints(
    ...
    if (amount > dailyLimit) {
      throw new Error(`За сутки можно выдать не более ${dailyLimit.toLocaleString("ru-RU")} баллов.`);
    }
      ...
      const dailyRow = this.db
        .prepare(
          `SELECT COALESCE(SUM(amount), 0) AS total
           FROM point_grants
           WHERE guild_id = ? AND issuer_id = ?
             AND created_at >= datetime('now', 'start of day')`,
        )
        .get(guildId, issuerId) as { total?: number } | undefined;
      const issuerDailyTotal = Number(dailyRow?.total ?? 0);
      if (issuerDailyTotal + amount > dailyLimit) {
        throw new Error(
          `Дневной лимит исчерпан. Уже выдано: ${issuerDailyTotal.toLocaleString("ru-RU")} из ${dailyLimit.toLocaleString("ru-RU")}.`,
        );
      }

      this.db
        .prepare(
          `INSERT INTO point_grants (guild_id, issuer_id, recipient_id, amount)
           VALUES (?, ?, ?, ?)`,
        )
        .run(guildId, issuerId, recipientId, amount);
      this.db
        .prepare(
          `INSERT INTO curator_points (guild_id, user_id, points)
           VALUES (?, ?, ?)
           ON CONFLICT(guild_id, user_id) DO UPDATE SET
             points = curator_points.points + excluded.points,
             updated_at = CURRENT_TIMESTAMP`,
        )
        .run(guildId, recipientId, amount);

      const recipientTotal = this.getPointBalance(guildId, recipientId).points;
      const newDailyTotal = issuerDailyTotal + amount;
      return {
        recipientTotal,
        issuerDailyTotal: newDailyTotal,
        issuerDailyRemaining: dailyLimit - newDailyTotal,
      };
    });
    return grant();
  }
